...
项目逻辑上被划分为三块：
远程服务 - 使用 HTTP/HTTPS,FTP与本地服务进行数据同步。
本地服务 - 基于远程服务和本地数据，为输入法端提供计算服务。
输入法端 - 使用 TCP 与本地服务进行状态同步。

远程服务设计模式和语言 MVC JAVA
本地服务设计模式和语言 MVVM C#
输入法端语言          C++

基于MVVM View和View-Model之间松耦合的特性，可将GUI和业务逻辑分离

因此本地服务端选用了MVVM，因为它包含了GUI。
